...
int main() {
    ...
    for (auto i = 1; i < 10; i++) {
        ...
        Mat q = Mat::identity(9);
        for (auto j = 0; j < 10; j++) {
            for (auto k = 0; k < 10; k++) {
                if (j == i || k == i) continue;
                q[j - (j > i)][k - (k > i)] -= trans[j][k];
            }
        }
        q = Mat::inv(q);

        // for (auto j = 0; j < 9; j++) {
        //     cout << "q[j]=["; for (auto __x:q[j])cout<<__x<<", "; cout<<"], "; cout << endl; // db I:q[j]
        // }
        for (auto j = 0; j < 9; j++)
            expect[i] += q[j][0];

        // cout<<"i="<<(i)<<", "; cout<<"expect[i]="<<(expect[i])<<", "; cout << endl; // db i,expect[i]
    }

    // cout<<"expect=["; for(int __i=0; __i<(10); __i++)cout<<(expect)[__i]<<", "; cout<<"], ";cout << endl; // db A:10:expect

    MI ans = 0;
    for (auto i = 0; i < SZ(a); i++) {
        int f = a[i]-'0', t = b[i]-'0', delta = f <= t ? t-f : 10-(f-t);
