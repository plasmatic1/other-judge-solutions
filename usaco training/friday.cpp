...
template <typename T> void printArray(T *arr,int sz){for(int i=0;i<sz-1;i++){cout<<arr[i]<<" "; }cout<<arr[sz - 1]<<"\n";}
...
int main(){
    ...
    int y = 1900, m = 0, d = 0, wkd = 2;
    while (y < n) {
        if (d == 12)
            cnt[wkd]++;

        // debug(y, m, d, wkd, dayCnt(y, m));
