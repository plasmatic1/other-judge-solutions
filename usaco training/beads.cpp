...
int same(int st, char c, int dir) {
    int cnt = 1;
    for (int i = st + dir; i >= 0 && i < n && s[i] == 'w' || s[i] == c; i += dir)
        cnt++;
    return cnt;
}
...
int main(){
    ...
    Fori(1, n) {
        ...
        if (c == 'w')
            cnt += max(same(i, 'r', 1), same(i, 'b', 1));
        else
            cnt += same(i, c, 1);
        
        if (d == 'w')
            cnt += max(same(i - 1, 'r', -1), same(i - 1, 'b', -1));
        else
            cnt += same(i - 1, d, -1);
