...
MI cntRectStart(MI h, MI w) {
    return w * sumArith(h);
}

MI cntRect(MI h, MI w) {
    return sumArith(h) * sumArith(w);
}
...
int main() {
    ...
    cin >> N;
    for (auto i = 1; i <= N; i++) cin >> H[i];
    for (auto i = 1; i <= N; i++) cin >> W[i];
    partial_sum(W, W+N+1, pw);

    deque<pair<MI, MI>> dq; dq.eb(0, 0);
    MI ans = 0, tot = 0;
    for (auto i = 1; i <= N; i++) {
        // remove incompatible
        MI h = H[i], w = W[i];
        while (!dq.empty() && H[i] < dq.back().fs) {
            tot -= cntRectStart(dq.back().fs, dq.back().sn);
            tot += cntRectStart(H[i], dq.back().sn);
            w += dq.back().sn;
