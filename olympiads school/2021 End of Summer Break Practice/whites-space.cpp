...
struct Comp {
    using Data = int;
    Data merge(Data a, Data b) { return S[a] < S[b] ? a : b; }
};
...
ll solve(int l, int r, int pre) {
    if (l > r) return 0;
    if (l == r) return S[l]-pre;
    int minIdx = st.query(l, r);
    return solve(l, minIdx-1, S[minIdx]) + solve(minIdx+1, r, S[minIdx]) + S[minIdx] - pre;
}
...
int main() {
    ...
    for (auto i = 1; i <= N; i++)
        cin >> S[i];
    {
        vector<int> _(N); iota(all(_), 1);
        st.init(all(_));
    }

    ll ans = solve(1, N, 0) + N-1;
    cout << (ans) << '\n';
