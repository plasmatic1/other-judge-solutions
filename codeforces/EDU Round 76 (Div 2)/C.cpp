...
int n,
    arr[MN]; //, cnt[MN];
vi ids[MN];
// multiset<int, greater<int>> freqs;
...
void solve() {
    ...
    Forinci(1, n) {
        if (ids[i].size() >= (size_t)2) {
            int sz = ids[i].size();
            Forj(0, sz - 1) {
                mina(minv, ids[i][j + 1] - ids[i][j] + 1);
            }
        }
    }

    println(minv == INF ? -1 : minv);    

    // int minv = INF;
    // int lptr = 1, rptr = 1;
    // for (; rptr <= n; rptr++) {
    //     auto ptr = freqs.find(cnt[arr[rptr]]);
    //     if (ptr != freqs.end()) freqs.erase(ptr);
    //     cnt[arr[rptr]]++;
    //     freqs.insert(cnt[arr[rptr]]);
