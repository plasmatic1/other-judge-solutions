...
struct pt {
    ll i, x, y;
    Cmplt(pt) { return x == o.x ? y < o.y : x < o.x; }
...
pair<ll, ll> slope(int i) {
    ll mn = pts[i + 1].y - pts[i].y;
    ll md = pts[i + 1].x - pts[i].x;
    ll gcdv = gcd(mn, md);
    if (md < 0) {
        md *= -1;
        mn *= -1;
    ...
    gcdv = abs(gcdv);
    return make_pair(mn / gcdv, md / gcdv);
}
...
int main(){
    ...
    Fori(0, n - 2) {
        ...
        if (pts[i].x == pts[i + 1].x && pts[i + 1].x == pts[i + 2].x) {
            continue;
        }
        else if (pts[i].x == pts[i + 1].x || pts[i + 1].x == pts[i + 2].x) {
            work = true;
        }
        else {
            work = slope(i) != slope(i + 1);
        }

        if (work) {
            println(pts[i].i, pts[i + 1].i, pts[i + 2].i);
            return 0;
        }
    }
