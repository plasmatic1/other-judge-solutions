...
const char T[63] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
const int MN = 101, MN2 = 10300;
int r, c, k,
    ans[MN][MN];
char grid[MN][MN];
...
void solve() {
    ...
    int amt = (trice + k - 1) / k, reqLess = amt * k - trice;
    Fori(reqLess, k) {
        sizes.pb(amt);
    }
    Fori(0, reqLess) {
    ...
    int tp = sizes.front(), mt = 1; sizes.pop_front();
    Fori(0, r) {
        int dir = i % 2 ? -1 : 1, st = i % 2 ? c - 1 : 0, en = i % 2 ? -1 : c;
        for (int j = st; j != en; j += dir) {
            tp -= grid[i][j] == 'R';
            if (tp < 0) {
                tp = sizes.front(); sizes.pop_front();
                tp--;
                mt++;
            }
