...
int powsUnder(int val, int bound) {
    int cur = 1, cnt = 0;
    // int it=10;
    while (cur <= bound) {
        // debug(cur,cnt,bound);
...
// int cc = 100;
void rec(int cur, ll num, int powc) {
    if (cur == sz) {
        if (powc == 0) return;
        // debug(num, powc);
    ...
    ll cpow = 1;
    for (int i = 0; i <= facts[cur].second; i++) {
        rec(cur + 1, num * cpow, powc + i);
        cpow *= facts[cur].first;
    }
}
...
int main(){
    ...
    for (int prime : primes) {
        if (prime > n) break;
        // facts.pb(mpr(prime, powsUnder(prime, n)));
        facts.pb(mpr(prime, 1));
        // debug(prime, powsUnder(prime, n));
    }
