...
bool validInter(int i, int j) {
    int num = abs(s[j] - s[i]), denom = abs(d[j] - d[i]);
...
int main(){
    ...
    Fori(0, n) {
        Forj(i + 1, n) {
            if (!validInter(i, j)) continue; // Only integer intersections
            int intX = inter(i, j), intY = solve(i, intX);
            if (intX < 1) continue; // only deal with integer intersections >= 1
            auto pr = mpr(intX, intY);
            cnt[pr]++;
        }
    }

    int best = 0, bTime = -1, bDis = -1;
    for (auto en : cnt) {
        auto loc = en.first;
        int cnt = pairMp[en.second];
        if (cnt > best) {
            best = cnt;
            bTime = loc.first;
            bDis = loc.second;
