...
int getMonthLen(int yr, int m) {
    if (m == 1) return MON[m] + isLeap(yr);
    return MON[m];
}

int monthLensBefore(int yr, int m) {
    int tot = 0;
    Fori(0, m - 1) {
        tot += getMonthLen(yr, i);
    }
    return tot;
}

int y, m, d, l[2], start[2];
