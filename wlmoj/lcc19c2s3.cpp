...
//         if (l >= ql && r <= qr) return LLINF;
//         if (l > qr || r < ql) 
//     }
// }
ll pfxMin[MN], sfxMin[MN];
...
int main(){
    ...
    Forinci(1, n) {
        pfx[i] = pfx[i - 1] + p[i] - c[i];
    }

    pfxMin[0] = LLINF;
    Forinci(1, n) {
        mina(pfxMin[i], min(pfxMin[i - 1], pfx[i]));
    }

    sfxMin[n + 1] = LLINF;
    for (int i = n; i >= 1; i--) {
        mina(sfxMin[i], min(sfxMin[i + 1], pfx[i]));
    }

    Forinci(1, n) {
        ll mina = sfxMin[i] - pfx[i - 1], minb = pfxMin[i - 1] + (pfx[n] - pfx[i - 1]);
        // debug(i, mina, minb, sfxMin[i], pfxMin[i - 1]);
        if (mina >= 0 && minb >= 0) {
            println(i);
            return 0;
        }
