...
const int MN = 1001, LG = 51;
ll n, m,
    arr[MN], isSet[LG], isNotSet[LG];
...
ll solve() {
    ...
    // First pass - Take all mins
    for (int i = LG - 1; i >= 0; i--) {
        ll v = 1LL << i, ifSet = isNotSet[i] * v, ifNotSet = isSet[i] * v;

        // debug(i, v, ifSet, ifNotSet);
        if (ifSet <= ifNotSet) {
            ctot += ifSet;
            k |= v;
        }
    ...
    // Send pass - add as much as possible
    for (int i = LG - 1; i >= 0; i--) {
        ll v = 1LL << i, ifSet = isNotSet[i] * v, ifNotSet = isSet[i] * v;

        // debug(i, v, isNotSet[i], isSet[i]);

        if (!((k >> i) & 1)) {
            ll add = ifSet - ifNotSet;
            if (ctot + add <= m) {
                ctot += add;
                k |= v;
            }
...
int main(){
    ...
    while (T--) {
        ...
        memset(isNotSet, 0, sizeof isNotSet);
