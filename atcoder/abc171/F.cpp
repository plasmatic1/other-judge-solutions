...
int main(){
    ...
    init_nchoosek(2000000);

    ll ans = 0;
    repi(0, K + 1) { // assume the string is N*'a', i -> The number of 'a's inserted
        int groupCount = (K - i) + 1, notACount = K - i;

        ll notA = mmul(fpow(25, notACount), choose(N + i + notACount, notACount));
        // db(i); db(notA); db(notACount); db(groupCount); db(fpow(25, notACount)); db(choose(N+notACount, notACount)); db(ans); dbln;
        ans = madd(ans, notA);
    }
    println(ans);
